    ...
    for xdist in (-1, 0, 1):
        for ydist in (-1, 0, 1):
            ...
            if neighbour[0] < 0 or neighbour[0] >= len(lines) or neighbour[1] < 0 or neighbour[1] >= len(lines[0]):
                continue
            diff = heights[current[0]][current[1]] - heights[neighbour[0]][neighbour[1]]
            if diff > 1:
                continue
            else:
                new_dist = old_dist + 1
                if new_dist < min_dist[neighbour[0]][neighbour[1]]:
                    ...
                    queue.sort()  
min_min_dist = len(lines[i]) * len(lines) + 1
for i in range(len(lines)): 
    for j in range(len(lines[i])):
        if min_dist[i][j] < min_min_dist and lines[i][j] == 'a':
            min_min_dist = min_dist[i][j]
print(min_min_dist)