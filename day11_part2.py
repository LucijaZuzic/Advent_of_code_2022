...
for i in range(monkey_num): 
    max_num = max(max_num, monkey_test[i])
    LCMresult *= monkey_test[i]

def are_all_divisible(divisor):
    for i in range(monkey_num):
        if monkey_test[i] % divisor != 0:
            return False
    return True 
 
for divisor in range(2, max_num):
    while are_all_divisible(divisor):
        LCMresult /= divisor
        for i in range(monkey_num): 
            monkey_test[i] /= divisor 
